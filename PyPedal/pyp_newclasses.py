...
class NewPedigree:
    ...
    def __init__(self, kw={}, kwfile='pypedal.ini'):
        ...
        :param kw: A dictionary of options.
        :param kwfile: An optionsl configuration file name.
        :return: An instance of a NewPedigree() object
        """
        ...
        if len(kw) == 0:
            ...
            if kw == {}:
                print '[ERROR]: pyp_newclasses.NewPedigree.__init__() was unable to load a configuration file ' \
                      'named %s!' % kwfile
                import sys
                sys.exit(0)
        ...
        if kw['simulate_pedigree']:
            ...
            if not kw.has_key('simulate_n'):
                kw['simulate_n'] = 15
            if not kw.has_key('simulate_g'):
                kw['simulate_g'] = 3
            if not kw.has_key('simulate_ns'):
                kw['simulate_ns'] = 4
        ...
        if not kw.has_key('sepchar'):
            ...
            kw['sepchar'] = ' '
        if not kw.has_key('alleles_sepchar'):
            kw['alleles_sepchar'] = '/'
        if not kw.has_key('counter'):
            kw['counter'] = 1000
        ...
        if not kw.has_key('missing_byear'):
            kw['missing_byear'] = 1900
        if not kw.has_key('missing_parent'):
            kw['missing_parent'] = 0
        if not kw.has_key('missing_name'):
            kw['missing_name'] = 'Unknown_Name'
        if not kw.has_key('missing_breed'):
            kw['missing_breed'] = 'Unknown_Breed'
        if not kw.has_key('missing_herd'):
            kw['missing_herd'] = 'Unknown_Herd'
        if not kw.has_key('missing_sex'):
            kw['missing_sex'] = 'u'
        if not kw.has_key('missing_inbreeding'):
            kw['missing_inbreeding'] = 0.
        if not kw.has_key('missing_genomic_inbreeding'):
            kw['missing_genomic_inbreeding'] = 0.
        if not kw.has_key('missing_alive'):
            kw['missing_alive'] = 0
        if not kw.has_key('missing_age'):
            kw['missing_age'] = -999
        if not kw.has_key('missing_gen'):
            kw['missing_gen'] = -999.
        if not kw.has_key('missing_gencoeff'):
            kw['missing_gencoeff'] = -999.
        if not kw.has_key('missing_igen'):
            kw['missing_igen'] = -999.
        if not kw.has_key('missing_pedcomp'):
            kw['missing_pedcomp'] = -999.
        if not kw.has_key('missing_alleles'):
            kw['missing_alleles'] = ['', '']
        if not kw.has_key('missing_userfield'):
            kw['missing_userfield'] = 'Unknown'
        ...
        if not kw.has_key('database_name'):
        ...
        if not kw.has_key('database_debug'):
            kw['database_debug'] = False
        if not kw.has_key('database_type'):
            kw['database_type'] = 'sqlite'
        if not kw.has_key('database_host'):
            kw['database_host'] = 'localhost'
        if not kw.has_key('database_user'):
            kw['database_user'] = 'anonymous'
        if not kw.has_key('database_passwd'):
            kw['database_passwd'] = 'anonymous'
        if not kw.has_key('database_port'):
            kw['database_port'] = ''
        if not kw.has_key('database_sql'):
            kw['database_sql'] = 'SELECT * FROM %s'
        if not kw.has_key('database_compatibility'):
            kw['database_compatibility'] = 'sqlite'
        ...
        if not kw.has_key('paper_size'):
            kw['paper_size'] = 'letter'
        if kw['paper_size'] == 'a4':
            kw['paper_size'] = 'A4'
        if kw['paper_size'] not in ['A4', 'letter']:
            kw['paper_size'] = 'letter'
        if not kw.has_key('default_unit'):
        ...
        if not kw.has_key('default_fontsize'):
        ...
        # reports to files.
        if not kw.has_key('default_report'):
        ...
        # Now that we have processed all the arguments in the options dictionary we need to attach it to this object.
        self.kw = kw

        # Initialize the Big Main Data Structures to null values
        self.pedigree = []  # We may start storing animals in a dictionary rather than in a
        # list.  Maybe,
        ...
        self.namemap = {}  # This is needed to map IDs to names when IDs are read using
        # string formats (ASD).
        self.backmap = {}
        self.namebackmap = {}
        ...
        logging.basicConfig(level=logging.DEBUG, format='%(asctime)s %(levelname)-8s %(message)s',
                            datefmt='%a, %d %b %Y %H:%M:%S', filename=self.kw['logfile'], filemode='w')
        ...
        try:
            ...
            logging.warning('An incorrect value (%s) was provided for the option log_ped_lines, which must be a number'
                            ' greater than or equal 0.  It has been set to 0.', _lpl)
            ...
            for a in self.pedigree:
                ...
                for b in other.pedigree:
                    ...
                    if mismatches == 0:
                        ...
                        if self.kw['debug_messages']:
                            print '[DEBUG]: Animals %s and %s are identical:' % (a.animalID, b.animalID)
                    else:
                        # Animals are different
                        ped_to_write['b'][b.animalID] = True
                        if self.kw['debug_messages']:
                            print '[DEBUG]: Animals %s and %s are different:' % (a.animalID, b.animalID)
                    if self.kw['debug_messages']:
    ...
    def intersection(self, other, newpedname='intersected_pedigree'):
        ...
        if self.__class__.__name__ == 'NewPedigree' and other.__class__.__name__ == 'NewPedigree':
            ...
            try:
                new_pedigree = loadPedigree(new_options, debugLoad=True)
                if self.kw['messages'] == 'verbose':
                    print '[INFO]: Loaded merged pedigree %s from file %s!' % (newpedname, filename)
                    logging.info('Cannot complete intersection operation because types do not match.')
                    return new_pedigree
            except:
                if self.kw['messages'] == 'verbose':
                    print '[ERROR]: Could not load merged pedigree %s from file %s!' % (newpedname, filename)
                logging.error('Could not load merged pedigree %s from file %s!', newpedname, filename)
                return False
        else:
    ...
    def load(self, pedsource='file', pedgraph=0, pedstream='', animallist=False):
        ...
        if pedsource not in ['file', 'db', 'graph', 'graphfile', 'null', 'animallist', 'gedcomfile',
                             'genesfile', 'textstream']:
            ...
            try:
                ...
                if pyp_db.doesTableExist(self):
                    ...
                    if conn:
                        ...
                        # sql = 'SELECT animaltattoo,if(siretattoo is NULL ,"",siretattoo),if(damtattoo is
                        # NULL ,"",damtattoo),if(concat(year(bdate)) is NULL ,"",concat(year(bdate))) FROM %s' %
                ...
                if self.kw['messages'] == 'verbose':
                    print '[ERROR]: Unable to load pedigree from database %s.%s' % \
                          (self.kw['database_name'], self.kw['database_table'])
                sys.exit(0)
            ...
            if animallist and len(animallist) > 0:
                try:
                    ...
                    if not animallist:
                        ...
                        logging.error('Unable to create a pedigree from an animal list because an empty list was'
                                      ' provided!')
                        sys.exit(0)
        ...
        # GEDCOM 5.5 standard.
            ...
            if self.kw['pedfile']:
                ...
                if pedformat != 'xxxx':
                    ...
                    logging.error('Unable to load pedigree from a GEDCOM file because an invalid pedigree format code,'
        ...
        # The genesfile pedsource reads files that conform to the dBase III format used by GENES v1.2.
            ...
            if self.kw['pedfile']:
                ...
                if pedformat != 'xxxx':
                    ...
                    logging.error('Unable to load pedigree from a GENES v1.20 file because an invalid pedigree format'
                                  ' code, %s, was returned!', pedformat)
            else:
                if self.kw['messages'] == 'verbose':
        ...
        # without worrying about Apache and file creation, etc. I'll see what I
        # can do...
    ...
    def oldsave(self, filename='', outformat='o', idformat='o'):
        ...
        if filename == '':
            ...
            if self.kw['messages'] == 'verbose':
                print '[WARNING]: Saving pedigree to file %s to avoid overwriting %s.' % \
                      (filename, self.kw['pedfile'])
    ...
    # @param append Add animal records to an existing file instead of creating a new one.
    ...
    def save(self, filename='', pedformat='asd', sepchar=' ', append=False, write_list=False, originalID=False):
        ...
        from the input pedigree file.
        """
        # Check the contents of the pedformat to make sure that each entry is valid. Save the pedformat passed in by
        # the user and add the valid entries a new version of pedformat which will be used for the remainder of the
        # processing.
        ...
        for pf in pedformat_in:
            if pf in self.pedformat_codes:
                ...
                if self.kw['messages'] == 'verbose':
                    print '[WARNING]: Invalid pedigree format code, %s, in NewPedigree::save(). Not included in ' \
                          'pedformat_ckd.' % pf
                logging.warning('Invalid pedigree format code, %s, in NewPedigree::save(). Not included in '
                                'pedformat_ckd.', pf)
        ...
        if ('ASD' not in pedformat) and ('asd' not in pedformat):
            if self.kw['messages'] == 'verbose':
                print '[WARNING]: The pedigree format code, %s, does not contain the sequence \'asd\' or \'ASD\', ' \
                      'and the resulting pedigree may have incomplete parentage information.' % pedformat
            logging.warning('The pedigree format code, %s, does not contain the sequence \'asd\' or \'ASD\', and the '
        ...
        if sepchar == '':
            ...
            logging.warning('Invalid sepchar \'\' in NewPedigree::save(). Changed to \'%s\' to keep columns from '
                            'running together.', sepchar)
        # This is VERY important: never overwrite the user's data if it looks like an accidental
        # request!  If the user does not pass a filename to save() save the pedigree to a file
        # whose name is derived from, but not the same as, the original pedigree file.
        if filename == '':
            filename = '%s_saved.ped' % (self.kw['filetag'])
            if self.kw['messages'] == 'verbose':
                print '[WARNING]: Saving pedigree to file %s to avoid overwriting %s.' % (filename,
                                                                                          self.kw['pedfile'])
            logging.warning('Saving pedigree to file %s to avoid overwriting %s.', filename, self.kw['pedfile'])
        try:
            if not append:
                ...
                ofh = file(filename, 'a')
                if self.kw['messages'] == 'verbose':
                    print '[INFO]: Opened file %s for pedigree save in append mode at %s.' % (filename,
            ...
            if append:
                ...
                ofh.write('#\tpedigree format: %s\n' % (self.kw['pedformat']))
            for _a in self.pedigree:
                # 06/23/2022 This looks a little janky...
    ...
    def savegraph(self, pedoutfile=0, pedgraph=0):
        ...
        :param pedoutfile: String containing the name of the output file.
        :return: True on success, False on failure.
        :param pedgraph:  NetworkX DiGraph object holding pedigree to save.
        """
        _retval = False
        try:
            if not pedoutfile:
    ...
    def savedb(self, drop=False):
        ...
        for NewAnimals and LightAnimals.
        :param self: NewPedigree object holding pedigree to save.
        :param drop: Boolean indicating if existing data should be kept (False) or deleted (True).
        :return: True on success, False on failure.
        """
        _retval = False
        try:
            ...
            if self.kw['database_compatibility'] == 'sqlite':
                ...
                if conn:
                    if not pyp_db.doesTableExist(self):
                        ...
                        if self.kw['messages'] == 'verbose':
                            print '[WARNING]: The table %s already exists in database %s and you told me to ' \
                                  'save the existing data. You may end up with duplicate data or multiple ' \
                                  'pedigrees stored in the same table!' % (self.kw['database_table'],
                                                                           self.kw['database_name'])
                        logging.warning('The table %s already exists in database %s and you told me to save '
                                        'the existing data. You may end up with duplicate data or multiple '
                                        'pedigrees stored in the same table!', self.kw['database_table'],
                                        self.kw['database_name'])
                    ...
                    if pyp_db.doesTableExist(self):
                        try:
                            for p in self.pedigree:
                                ...
                                sql = "INSERT INTO %s ( animalName, sireName, damName, sex ) VALUES ('%s', " \
                                      "'%s', '%s', '%s')" % (self.kw['database_table'], an, si, da, p.sex)
                                cursor = conn.cursor()
                                cursor.execute(sql)
                                cursor.commit()
                            _table_loaded = True
            ...
            #
                ...
                # Create the database -- this will overwrite an existing DB with the
                # same name!
                if pyp_db.doesTableExist_adodb(self) and drop:
                    ...
                    if not pyp_db.doesTableExist_adodb(self):
                        ...
                        if self.kw['messages'] == 'verbose':
                            print '[WARNING]: The table %s already exists in database %s and you told me to save the' \
                                  ' existing data. You may end up with duplicate data or multiple pedigrees stored ' \
                                  'in the same table!' % (self.kw['database_table'], self.kw['database_name'])
                        logging.warning('The table %s already exists in database %s and you told me to save the '
                                        'existing data. You may end up with duplicate data or multiple pedigrees '
                                        'stored in the same table!', self.kw['database_table'],
                                        self.kw['database_name'])
                    # Load the pedigree data into the table.
                    if pyp_db.doesTableExist_adodb(self):
                        try:
                            for p in self.pedigree:
                                ...
                                if da == self.kw['missing_name']:
                                    da = self.kw['missing_parent']
                                sql = "INSERT INTO %s ( animalName, sireName, damName, sex ) VALUES ('%s', '%s', " \
                                      "'%s', '%s')" % (self.kw['database_table'], an, si, da, p.sex)
                                cursor = conn.Execute(sql)
                                cursor.Close()
                            _table_loaded = True
                        except:
    ...
    def preprocess(self, textstream='', dbstream=''):
        ...
        try:
            if not self.kw['pedformat']:
                ...
                if self.kw['messages'] == 'verbose':
                    print '[ERROR]: Null pedigree format string assigned a default value of %s.' % \
                          self.kw['pedformat']
            ...
            for _char in self.kw['pedformat']:
                if _char in self.pedformat_codes and _char != 'Z':
                    ...
                    logging.info('Skipping one or more columns in the input file as requested by the pedigree '
                                 'format string %s', self.kw['pedformat'])
                else:
            ...
            for _char in _pedformat:
                try:
                    ...
                    try:
                        ...
                        print '[CRITICAL]: No animal identification code was specified in the pedigree format string' \
                              ' %s! This is a critical error and the program will halt.' % _pedformat
                ...
                try:
                    ...
                    try:
                        ...
                        print '[CRITICAL]: No sire identification code was specified in the pedigree format string ' \
                              '%s! This is a critical error and the program will halt.' % _pedformat
                        critical_count = critical_count + 1
                try:
                    ...
                    try:
                        ...
                        print '[CRITICAL]: No dam identification code was specified in the pedigree format string ' \
                              '%s! This is a critical error and the program will halt.' % _pedformat
                        critical_count = critical_count + 1
                try:
                    ...
                    if self.kw['messages'] == 'all':
                        print '[DEBUG]: No generation code was specified in the pedigree format string %s. This ' \
                ...
                try:
                    ...
                    if self.kw['gen_coeff']:
                        self.kw['gen_coeff'] = 0
                    if self.kw['messages'] == 'all':
                        print '[DEBUG]: No generation coefficient was specified in the pedigree format string %s. ' \
                ...
                try:
                    ...
                    if self.kw['messages'] == 'all':
                        print '[DEBUG]: No sex code was specified in the pedigree format string %s. This program ' \
                ...
                try:
                    ...
                    if self.kw['messages'] == 'all':
                        print '[DEBUG]: No birth date (YYYY) code was specified in the pedigree format string %s. ' \
                              'This program will continue.' % self.kw['pedformat']
                try:
                    ...
                    if self.kw['messages'] == 'all':
                        print '[DEBUG]: No coeffcient of inbreeding code was specified in the pedigree format ' \
                              'string %s. This program will continue.' % self.kw['pedformat']
                try:
                    pedformat_locations['genomic_inbreeding'] = _pedformat.index('G')
                except ValueError:
                    pedformat_locations['genomic_inbreeding'] = -999
                    if self.kw['messages'] == 'all':
                        print '[DEBUG]: No coeffcient of genomic inbreeding code was specified in the pedigree ' \
                              'format string %s. This program will continue.' % self.kw['pedformat']
                try:
                ...
                try:
                    ...
                    if self.kw['messages'] == 'all':
                        print '[DEBUG]: No breed code was specified in the pedigree format string %s. This program ' \
                              'will continue.' % self.kw['pedformat']
                try:
                    ...
                    if self.kw['messages'] == 'all':
                        print '[DEBUG]: No name code was specified in the pedigree format string %s. This program ' \
                              'will continue.' % self.kw['pedformat']
                try:
                    ...
                    if self.kw['messages'] == 'all':
                        print '[DEBUG]: No birth date (MMDDYYYY) code was specified in the pedigree format string ' \
                              '%s. This program will continue.' % self.kw['pedformat']
                try:
                    pedformat_locations['alive'] = _pedformat.index('l')
                except ValueError:
                    pedformat_locations['alive'] = -999
                    if self.kw['messages'] == 'all':
                        print '[DEBUG]: No alive/dead code was specified in the pedigree format string %s. This ' \
                              'program will continue.' % self.kw['pedformat']
                try:
                    ...
                    if self.kw['messages'] == 'all':
                        print '[DEBUG]: No age code was specified in the pedigree format string %s. This program ' \
                              'will continue.' % self.kw['pedformat']
                try:
                    ...
                    pedformat_locations['alleles'] = -999
                    if self.kw['messages'] == 'all':
                        print '[DEBUG]: No alleles code was specified in the pedigree format string %s. This ' \
                              'program will continue.' % self.kw['pedformat']
                try:
                    ...
                    if self.kw['messages'] == 'all':
                        print '[DEBUG]: No herd code was specified in the pedigree format string %s. This program ' \
                              'will continue.' % self.kw['pedformat']
                try:
                    pedformat_locations['herd'] = _pedformat.index('H')
                except ValueError:
                ...
                #     if self.kw['messages'] == 'all':
                #         print '[DEBUG]: No SNP genotype field was specified in the pedigree format string %s. This
                #         program will continue.' % (self.kw['pedformat'])
                ...
                while 1:
                    ...
                    if not line:
                        ...
                        if lineCounter == 1 and self.kw['has_header']:
                            ...
                            if self.kw['messages'] == 'verbose' and self.kw['pedigree_summary']:
                                print '[INFO]: Converted the first line in the input file into a comment because ' \
                                      'the pedigree file has a header row.'
                            line = '# %s' % line
                        ...
                        # format code.
                            ...
                            self.kw['old_pedformat'] = string.strip(line[1:])  # this lets us specify the format of
                            # the pedigree file
                            logging.warning('Encountered deprecated pedigree format string (%s) on line %s of the '
                                            'pedigree file.', line, lineCounter)
                        ...
                        # catches blank lines so that they are not treated as actual records, and logs them.
                            ...
                            lfields = string.split(string.strip(line), self.kw['sepchar'])
                            ...
                            if (len(self.kw['pedformat']) == len(lfields)) or \
                                    (len(self.kw['pedformat']) == len(lfields) + 1 and 'P' in self.kw['pedformat']):
                                self.namemap = {}
                                self.namebackmap = {}
                                # print line
                                # print lfields
                                ...
                                if len(lfields) < 3:
                                    errorString = 'The record on line %s of file %s is too short - all records must ' \
                                                  ...
                                                  (lineCounter, self.kw['pedfile'], len(lfields))
                                    ...
                                    if lfields[0] != self.kw['missing_parent']:
                                        ...
                                        # 06/23/2022 Is there a logic error here? Shouldn't this break out of the
                                        # current iteration of input to actually skip processing this animal?
                                        errorString = 'The record on line %s of file %s has an animal ID that is ' \
                                                      'the same as the missing value code specified for the ' \
                                                      'pedigree. This animal is being skipped and will not have an ' \
                                                      'entry in the pedigree.\n' % \
                                                      (lineCounter, self.kw['pedfile'])
                                        print '[ERROR]: %s' % errorString
                                        logging.error(errorString)
                                    # If strings are used for sire and dam IDs we need to put the names in the
                                    # _sires or _dams dictionary rather than the ID.  If we put in the IDs the
                                    # missing sire and dam catcher code will not work correctly.
    ...
    def fromgraph(self, pedgraph):
        ...
        DiGraph object.
        :param self: NewPedigree object.
        :param pedgraph: NetworkX DiGraph object.
        :return: True.
        """
        _retval = False
        missing = ['sex', 'generation', 'gencoeff', 'birthyear', 'inbreeding', 'breed', 'name', 'birthdate',
                   'alive', 'age', 'alleles', 'herd', 'userfield']
        pedformat_locations = {
            'animal': 0,
            'sire': 1,
            'dam': 2,
        ...
        try:
            ...
            if self.kw['debug_messages']:
                logging.error('Added %s animals to the pedigree in pyp_newclasses::NewPedigree/fromgraph()!' %
                              len(pedgraph.nodes()))
            if self.kw['messages']:
                print '[INFO]: Added %s animals to the pedigree in pyp_newclasses::NewPedigree/fromgraph()!' % \
                      len(pedgraph.nodes())
            _retval = True
        except:
            if self.kw['debug_messages']:
    ...
    def addanimal(self, animalID, sireID, damID):
        ...
        addanimal() adds a new animal of class NewAnimal to the pedigree.
        :param self: NewPedigree object.
        :param animalID: animalID ID of the new animal to be added to the pedigree.
        :param sireID: Sire ID of the new animal to be added to the pedigree.
        :param damID: Dam ID of the new animal to be added to the pedigree.
        :return: True on success, False on failure.
        """
    ...
    def delanimal(self, animalID):
        ...
        than directly by users.
        :param self: NewPedigree object.
        :param animalID: ID of animal to delete.
        :return: True on success, False on failure.
        """
        _deleted = False
        if not self.kw['pedigree_is_renumbered']:
            logging.warning('Deleting an animal from an unrenumbered pedigree using NewPedigree::delanimal() is '
                            'unsafe!')
        try:
    ...
    # pedigree metadata are updates. This is intended for use by the "public" instead of delanimal(), which is
    ...
    def delete_animals(self, animal_list):
        ...
        a potential source of catastrophe.
        :param self: NewPedigree object.
        :param animal_list: List of animal IDs to delete.
        :return: True on success, False on failure.
        """
        _deleted = False
        try:
            ...
            if self.kw['resolve_duplicates']:
                ...
                self.duplicates = False
            if self.duplicates:
                print '[WARNING]: There are duplicate animals in the pedigree file, this requires manual ' \
    ...
    def simulate(self):
        ...
        Matvec 1.1a (src/classes/pedigree.cpp), although all the code in
        ...
        if len(self.pedigree) > 0:
            logging.error('The simulate() method did not create a new randomly-generated pedigree because the '
                          'pedigree %s has already been populated with animals.', self.kw['pedname'])
            if self.kw['messages'] == 'verbose':
                print '[ERROR]: The simulate() method did not create a new randomly-generated pedigree because ' \
                      'the pedigree %s has already been populated with animals.' % (self.kw['pedname'])
        ...
        if self.kw['simulate_n'] < 1:
            logging.warning('You asked that a pedigree containing an invalid number of animals, %s, be simulated. '
                            'The default number of animals, 15, is being used.', self.kw['simulate_n'])
            if self.kw['messages'] == 'verbose':
        ...
        if self.kw['simulate_g'] < 1:
            logging.warning('You asked that a pedigree containing an invalid number of generations, %s, be '
                            'simulated. The default number of generations, 3, is being used.', self.kw['simulate_g'])
            if self.kw['messages'] == 'verbose':
                print '\t[SIMULATE]: You asked that a pedigree containing an invalid number of generations, %s, ' \
                      'be simulated. The default number of generations, 3, is being used.' % (self.kw['simulate_g'])
            self.kw['simulate_g'] = 3
        if self.kw['simulate_ns'] < 1:
            logging.warning('You asked that a pedigree containing an invalid number of sires, %s, be simulated. '
                            'The default number of sires, 4, is being used.', self.kw['simulate_ns'])
            if self.kw['messages'] == 'verbose':
        ...
        if self.kw['simulate_nd'] < 1:
            logging.warning('You asked that a pedigree containing an invalid number of dams, %s, be simulated. The '
                            'default number of dams, 4, is being used.', self.kw['simulate_nd'])
            if self.kw['messages'] == 'verbose':
        ...
        if self.kw['simulate_sr'] < 0. or self.kw['simulate_sr'] > 1.:
            logging.warning('You asked that a pedigree with an invalid sex ratio, %s, be simulated. The default '
                            'sex ratio, 0.5, is being used.', self.kw['simulate_sr'])
            if self.kw['messages'] == 'verbose':
        ...
        if self.kw['simulate_ir'] < 0. or self.kw['simulate_ir'] > 1.:
            logging.warning('You asked that a pedigree with an invalid immigration rate, %s, be simulated. The '
                            'default rate, 0.5, is being used.', self.kw['simulate_ir'])
            if self.kw['messages'] == 'verbose':
        ...
        if self.kw['simulate_ns'] >= self.kw['simulate_n']:
            logging.error('You asked that a pedigree with more founder sires than total animals be simulated. '
        ...
        if self.kw['simulate_nd'] >= self.kw['simulate_n']:
            logging.error('You asked that a pedigree with more founder dams than total animals be simulated. '
                          'This is a fatal error!')
            if self.kw['messages'] == 'verbose':
        ...
        if (self.kw['simulate_ns'] + self.kw['simulate_ns']) > self.kw['simulate_n']:
            logging.error('You asked that a pedigree with more founder parents than total animals be simulated. '
                          'This is a fatal error!')
            if self.kw['messages'] == 'verbose':
        ...
        if self.kw['simulate_pmd'] < 1:
            logging.warning('You specified an invalid number of maximum parent draws, %s. The default number of '
                            'maximum draws, 100, is being used.', self.kw['simulate_pmd'])
            if self.kw['messages'] == 'verbose':
                print '\t[SIMULATE]: You specified an invalid number of maximum parent draws, %s. The default number ' \
                      'of maximum draws, 100, is being used.' % (self.kw['simulate_pmd'])
        ...
        for g in range(_sng):
            ...
            for _j in range(_npg):
                if _totalna >= _snt:
                    break
                # If a random uniform variate is greater than the immigration rate, we need to create a new animal
                # with a sire and a dam chosen at random from the males and females dictionaries, respectively.
                # if random.random() > _sir:
                if numpy.random.ranf() > _sir:
                    # The number-of-tries counter is used to track the number of draws needed to get
                    # a set of parents which meets all the MP, OP, and FS restrictions.
                    _ntry = 0
                    while 1:
                        # print 'females:\t%s' % ( females )
                        # print 'males:\t\t%s' % ( males )
                        ...
                        # In the Matvec code, when an unknown sire or dam is selected their sire (dam) is set the
                        # same ID as the "missing parent" flag. This results in a slight bias in favor of the
                        # first non-missing sire (dam) ID. It also means that the definition of the missing
                        # parent flag as described on p. 12 of the manual is counterintuitive -- 0 indicates
                        # that missing parents are allowed, not 1. I've fixed that here in PyPedal: an MP flag of
                        # 1 indicates that MP are allowed. That's the reason for the "1 - _smp" below.
                        if _selsire == 0:
                            _selsire = 1 - _smp
                        if _seldam == 0:
                            _seldam = 1 - _smp
                        # Now we're going to store the sire and dam IDs, as well as their parents' IDs, for use in
                    ...
                    # The new animal is an immigrant with unknown parents.
                    _selsire = 0
                    _seldam = 0
        ...
        for _ip in _pedholder:
            if _ip:
                # We don't want to include our missing parents in the final pedigree.
                if _ip.animalID != 0:
                    # _ip.printme()
                    lightans = []
                    lightans.append(str(_ip.animalID))
                    # 0 is used in the simulate routine to indicate a missing parent, but the user may have
                    # specified a different MP indicator.
                    if _ip.sireID == 0:
                        lightans.append(self.kw['missing_parent'])
                    else:
                        lightans.append(str(_ip.sireID))
                    if _ip.damID == 0:
                        lightans.append(self.kw['missing_parent'])
                    else:
                        lightans.append(str(_ip.damID))
                    lightans.append(_ip.sex)
                    lightans.append(_ip.gen)
                    if self.kw['animal_type'] == 'light':
                        an = LightAnimal(pedformat_locations, lightans, self.kw)
                        # an.printme()
                        self.pedigree.append(an)
                        self.idmap[an.animalID] = an.animalID
                        self.backmap[an.animalID] = an.animalID
                        ...
                        an = NewAnimal(pedformat_locations, lightans, self.kw)
                        # an.printme()
                        self.pedigree.append(an)
                        self.idmap[an.animalID] = an.animalID
                        self.backmap[an.animalID] = an.animalID
...
class NewAnimal:
    ...
    def __init__(self, locations, data, mykw):
        ...
        if locations['alleles'] != -999:
            self.alleles = [string.split(data[locations['alleles']], mykw['alleles_sepchar'])[0],
                            string.split(data[locations['alleles']], mykw['alleles_sepchar'])[1]]
        else:
    ...
    def __equals__(self, other, debug_messages=False):
    ...
    def dictme(self):
        ...
        try:
            _dict = {
                ...
                'fa': self.fa,
                'fg': self.fg,
                'homozygosity': self.homozygosity,
...
class LightAnimal:
    ...
    def pad_id(self):
        ...
        count of zeros, zeros.
        """
        lpad = len(self.animalID)
        pl = 15 - lpad - 1
        if pl > 0:
            zs = '0' * pl
            pid = '%s%s%s%s' % (self.by, zs, self.animalID, lpad)
        else:
            pid = '%s%s%s' % (self.by, self.animalID, lpad)
        return pid
...
class PedigreeMetadata:
    """A class to hold metadata about pedigrees.  Hopefully this will help improve performance in some procedures,
...
def loadPedigree(options='', optionsfile='pypedal.ini', pedsource='file', pedgraph=0, pedstream='', debugLoad=False):
    ...
    :param pedstream: String of tuples to unpack into a pedigree.
    :param debugLoad: When True, print debugging messages while loading (True|False).
    :return: An instance of a NewPedigree object on success, a False on failure.
    """
    if debugLoad:
