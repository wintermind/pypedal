...
class NewPedigree:
    ...
    def __init__(self, kw={}, kwfile='pypedal.ini'):
        ...
        # propagate back to the used, but __init__() methods can only
        ...
        # pedfile.
        if 'simulate_pedigree' not in kw.keys():
        ...
        if 'file_io' not in kw.keys():
            kw['file_io'] = '1'
        if 'debug_messages' not in kw.keys():
            kw['debug_messages'] = 0
        if 'form_nrm' not in kw.keys():
            kw['form_nrm'] = 0
        if 'nrm_method' not in kw.keys():
            kw['nrm_method'] = 'nrm'
        if 'nrm_format' not in kw.keys():
            kw['nrm_format'] = 'text'
        if 'f_computed' not in kw.keys():
            kw['f_computed'] = 0
        if 'long_ped_lines' not in kw.keys():
            kw['log_ped_lines'] = 0
        if 'log_long_filename' not in kw.keys():
            kw['log_long_filenames'] = 0
        if 'pedigree_summary' not in kw.keys():
            kw['pedigree_summary'] = 1
        if kw['pedigree_summary'] not in [0, 1, 2]:
            kw['pedigree_summary'] = 1
        if 'animal_type' not in kw.keys():
            kw['animal_type'] = 'new'
        ...
        if 'database_name' not in kw.keys():
            kw['database_name'] = 'pypedal'
        if 'database_table' not in kw.keys():
        ...
        if 'database_debug' not in kw.keys():
            kw['database_debug'] = False
        if 'database_type' not in kw.keys():
            kw['database_type'] = 'sqlite'
        if 'database_host' not in kw.keys():
            kw['database_host'] = 'localhost'
        if 'database_user' not in kw.keys():
            kw['database_user'] = 'anonymous'
        if 'database_passwd' not in kw.keys():
            kw['database_passwd'] = 'anonymous'
        if 'database_port' not in kw.keys():
            kw['database_port'] = ''
        if 'database_sql' not in kw.keys():
        ...
        if 'paper_size' not in kw.keys():
            kw['paper_size'] = 'letter'
        if kw['paper_size'] == 'a4':
            kw['paper_size'] = 'A4'
        if kw['paper_size'] not in ['A4', 'letter']:
            kw['paper_size'] = 'letter'
        if 'default_unit' not in kw.keys():
        ...
        if 'default_fontsize' not in kw.keys():
        ...
        # reports to files.
        if 'database_report' not in kw.keys():
        ...
        if 'newanimal_caller' not in kw.keys():
            kw['newanimal_caller'] = 'loader'

        # Options related to traits
        if 'trait_names' not in kw.keys():
            kw['trait_names'] = []
        if 'trait_auto_name' not in kw.keys():
            kw['trait_auto_name'] = 1
        if 'trait_count' not in kw.keys():
            kw['trait_count'] = 0

        # We need a match rule to use with the __add__() method.
        if 'match_rule' not in kw.keys():
            kw['match_rule'] = 'asd'
            ...
            while 1:
                ...
                if not line:
                    ...
                    # format code.
                        ...
                        self.kw['old_pedformat'] = string.strip(
                            line[1:])  # this lets us specify the format of the pedigree file
                        logging.warning(
