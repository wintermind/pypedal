...
# @retval False for failure and True for success.
...
def set_generation(pedobj):
    ...
    try:
        ...
        for i in range(pedobj.metadata.num_records):
            if pedobj.pedigree[i].sireID == pedobj.kw['missing_parent'] and pedobj.pedigree[i].damID == \
...
def set_age(pedobj):
    ...
    try:
        ...
        l = len(pedobj.pedigree)
        for i in range(l):
            if pedobj.pedigree[i].by == pedobj.kw['missing_value'] and pedobj.pedigree[i].igen == \
                    pedobj.kw['missing_value']:
                pedobj.pedigree[i].age = pedobj.kw['missing_value']
            elif pedobj.pedigree[i].by == pedobj.kw['missing_value'] and pedobj.pedigree[i].igen != \
                    -pedobj.kw['missing_value']:
...
def reorder(myped, filetag='_reordered_', io='no', missingparent=0, debug=0, max_rounds=100):
    ...
    while 1:
        ...
        if debug:
            print '_last_founder: ', _last_founder
        for i in xrange(l):
            #
...
def fast_reorder(myped, filetag='_new_reordered_', io='no', debug=0):
    """
    Renumbers a pedigree such that parents precede their offspring in the pedigree. In order to minimize overhead as
    much as is reasonably possible, a list of animal IDs that have already been seen is kept. Whenever a parent that
    is not in the seen list is encountered, the offspring of that parent is moved to the end of the pedigree. This
    should ensure that the pedigree is properly sorted such that all parents precede their offspring. myped is
    reordered in place. fast_reorder() uses dictionaries to renumber the pedigree based on paddedIDs.
...
# @param outformat Flag to indicate whether or not to write an asd pedigree (False) or a full pedigree (True).
...
# @retval A reordered PyPedal pedigree.
def renumber(myped, filetag='_renumbered_', io='no', outformat='0', debug=False, returnmap=False, missingparent=False,
    ...
    :param animaltype: Type of animal object used to populate pedigree.
    :param cleanmap: Cleans-up (deletes) the ID map file after renumbering is complete.
    :return: A reordered PyPedal pedigree, or a reorderd pedigree and an ID map.
    """
    if debug:
...
# @retval A dictionary whose keys are renumbered IDs and whose values are original IDs or an empty dictionary (on
#         failure).
...
def delete_id_map(filetag='_renumbered_'):
    ...
    :param filetag: A descriptor prepended to output file names.
    :return: True if success and False on failure.
    """
    try:
        _infile = '%s%s%s' % (filetag, '_id_map', '.map')
...
def trim_pedigree_to_year(pedobj, year):
    ...
    # The reduced (trimmed) pedigree is returned.
    try:
        indices = []
...
def pedigree_range(pedobj, n):
    """
    pedigree_range() takes a renumbered pedigree and removes all individuals with a renumbered ID > n. The reduced
    pedigree is returned. Assumes that the input pedigree is sorted on animal key in ascending order.
    :param pedobj: A PyPedal pedigree object.
    :param n: A renumbered animalID.
...
def sort_dict_by_keys(mydict):
    """
    sort_dict_by_keys() returns a dictionary where the values in the dictionary in the order obtained by sorting
    the keys. Taken from the routine sortedDictValues3 in the "Python Cookbook", p. 39.
    :param mydict: A non-empty Python dictionary.
...
# http://groups-beta.google.com/group/comp.lang.python/browse_thread/thread/bbc259f8454e4d3f/
# cc686f4cd795feb4?q=python+%22sorted+dictionary%22&rnum=1&hl=en#cc686f4cd795feb4
...
def simple_histogram_dictionary(mydict, histchar='*', histstep=5):
    ...
    :param mydict: Python list of dictionary values.
    :param histchar: The character used to draw the histogram (default is '*').
    :param histstep: Used to determine the number of bins (stars) in the diagram.
...
# @retval The input string with the order of its characters reversed, of False (on failure).
def reverse_string(mystring):
    """
    reverse_string() reverses the input string and returns the reversed version.
    :param mystring: A non-empty Python string.
    :return: The input string with the order of its characters reversed, of False (on failure).
    """
    try:
        if len(mystring) < 2:
            return mystring
...
# @retval A 14-character string containing the datestamp, or False (on failure).
def pyp_datestamp():
    """
    pyp_datestamp() returns a datestamp, as a string, of the format YYYYMMDDHHMMSS.
...
# @retval A dictionary whose keys are founder alleles and whose values are 0.0, or an empty dictionary (on failure).
...
# @param pedobj A PyPedal pedigree.
# @retval A dictionary whose keys are founder alleles and whose values are the founder of origin, or an empty
...
def founder_allele_map(pedobj):
    ...
    mapping founder alleles to their animal of origin.
    :param pedobj: PyPedal pedigree.
    :return: A dictionary whose keys are founder alleles and whose values are the founder of origin, or an empty
             dictionary (on failure).
    """
    try:
        falist = [pedobj.pedigree[x-1].alleles for x in pedobj.metadata.unique_founder_list]
...
def list_intersection(pedobjs):
    ...
    of false is returned.
    :param pedobjs: A list of PyPedal pedigrees.
    :return: A new PyPedal pedigree containing the animals that are common to both input pedigrees.
    """
    intersected = []
    if len(pedobjs) > 0:
        ...
        logging.error('An empty list was passed to pyp_utils/list_intersection()! Cannot compute intersection on 0 '
                      'pedigrees.')
...
# input pedigrees.
# @param pedobjs A list of PyPedal pedigrees.
# @retval A new PyPedal pedigree containing the animals that are included in either or both input pedigrees.
def list_union(pedobjs):
    """
    list_union() returns a PyPedal pedigree object which contains all animals included in either or both of the
    input pedigrees.
    :param pedobjs: A list of PyPedal pedigrees.
    :return: A new PyPedal pedigree containing the animals that are included in either or both input pedigrees.
    """
    if len(pedobjs) > 0:
    ...
    if animal.__class__.__name__ == 'NewAnimal':
        ...
        if isinstance(animal.originalID, int):
            ...
            if ped_kw['messages'] == 'verbose':
                print '[ERROR]: pyp_utils/guess_pedformat() cannot process animal.originalID if it is not a string ' \
                      'or an integer type!'
            return False
...
def list_duplicates(pedobj, keep_rule=''):
    ...
    :return: A list of duplicates animals to delete.
    """
    ...
    for k, v in duplicate_id_count.iteritems():
        if pedobj.kw['messages'] == 'verbose':
            print '\t[INFO]: pyp_utils/resolve_duplicates(): originalID %s occurs %s times in the pedigree file!' % \
                  (k, v)
...
# @retval The name of the program, or False (on failure).
def which(program):
    """
...
# @param pedobj PyPedal pedigree object.
# @retval An instance of a NewPedigree object with no animals with missing-valued , or False (on failure).
